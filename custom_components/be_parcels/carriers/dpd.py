...
LET OP — Cloudflare: DPD's site draait achter Cloudflare (Turnstile-
scripts zichtbaar in de pagina). Verzoeken zonder browser-achtige
headers gaven een 403 terug; deze module stuurt daarom een
User-Agent/Accept/Accept-Language die op een gewone browser lijkt. Als
dat op termijn tóch geblokkeerd wordt (bv. bij veel gelijktijdige
gebruikers), is een JS-uitvoerende aanpak (headless browser) nodig —
niet haalbaar binnen een Home Assistant-integratie zonder extra
afhankelijkheden.
"""
...
# Zonder geloofwaardige browser-headers blokkeert DPD's Cloudflare-
# bescherming het verzoek met een 403, ook al is de data zelf publiek
# (geen login nodig). Dit bootst een gewone browser-request na.
REQUEST_HEADERS = {
    "User-Agent": (
...
WARMUP_URL = "https://www.dpdgroup.com/be/mydpd/my-parcels/incoming?lang=nl"
...
class DpdCarrier(ParcelCarrier):
    ...
    async def async_get_status(
        ...
        try:
            # Opwarmverzoek: een gewone browser bezoekt nooit rechtstreeks
            # een diep gelinkte pagina zonder eerst iets anders geladen te
            # hebben. Dit haalt Cloudflare-cookies op via dezelfde sessie
            # (aiohttp bewaart cookies automatisch binnen één ClientSession),
            # wat de kans op een 403 op het echte verzoek verkleint.
            async with self._session.get(
                WARMUP_URL, headers=REQUEST_HEADERS, timeout=15
            ):
                pass  # resultaat niet nodig, enkel de cookies tellen
            ...
            async with self._session.get(
                ...
                if resp.status == 403:
                    raise ParcelProviderError(
                        "DPD blokkeerde het verzoek (403), zelfs met browser-"
                        "headers en een opwarmverzoek. Dit wijst op een stevigere "
                        "Cloudflare-blokkade die niet met eenvoudige middelen te "
                        "omzeilen is — zie caveat bovenaan dpd.py."
                    )
                if resp.status != 200:
