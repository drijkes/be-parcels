...
Gebruikt altijd de 'verify'-methode (trackingnummer + postcode) — de
brontekst gebruikte enkel 'search' in het voorbeeld dat we bekeken,
'verify' is er wel in gedocumenteerd maar niet apart bevestigd te werken.
...
hieronder staat — pas dan VERIFY_URL of de request-vorm aan.
"""
...
VERIFY_URL = "https://www.mydpd.be/jws.php/parcel/verify"
...
class DpdCarrier(ParcelCarrier):
    slug = "dpd"
    name = "DPD (BE)"
    # Gebruikt altijd de preciezere 'verify'-methode (trackingnummer +
    # postcode) i.p.v. de losse 'search' — postcode is dus verplicht.
    ...
    async def async_get_status(
        ...
        if not postal_code:
            raise ParcelProviderError(
                "DPD heeft de postcode van de ontvanger nodig om te kunnen tracken."
            )

        try:
            async with self._session.post(
                VERIFY_URL, json=[tracking_number, postal_code], timeout=15
            ) as resp:
                if resp.status == 404:
