...
(of de service be_parcels.add_parcel). Hier stel je in:
  ...
  - je (gratis) 17TRACK API-key, nodig voor alle vervoerders behalve
    bpost (zie carriers/aggregator.py).
"""
...
from .const import CONF_NOTIFY_SERVICES, CONF_SEVENTEENTRACK_API_KEY, DOMAIN
    ...
    return vol.Schema(
        {
            ...
            vol.Optional(CONF_SEVENTEENTRACK_API_KEY, default=current_key): selector.selector(
                {"text": {"type": "password"}}
            ),
        }
    )
        ...
        if user_input is not None:
            return self.async_create_entry(
                ...
                options={
                    ...
                    CONF_SEVENTEENTRACK_API_KEY: user_input.get(CONF_SEVENTEENTRACK_API_KEY, ""),
                },
            )
...
class BeParcelsOptionsFlow(config_entries.OptionsFlow):
    """Laat toe notify-doelen en de 17TRACK-key achteraf te wijzigen."""
    ...
    async def async_step_init(
        ...
        if user_input is not None:
            ...
            # getrackte pakjes) — enkel deze velden overschrijven, niet
            # de volledige options-dict vervangen.
            new_options = {**self._config_entry.options, **user_input}
            return self.async_create_entry(title="", data=new_options)

        current_notify = list(self._config_entry.options.get(CONF_NOTIFY_SERVICES, []))
        current_key = self._config_entry.options.get(CONF_SEVENTEENTRACK_API_KEY, "")
        return self.async_show_form(
