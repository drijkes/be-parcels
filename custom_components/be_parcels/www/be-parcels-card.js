class BeParcelsCard extends HTMLElement {
  static CARRIERS = [
    ...
    { slug: "dpd", label: "DPD (EU)", implemented: true },
  ...
  setConfig(config) {
    ...
    this._carrier = "bpost";
    this._postalCode = this._loadLastPostalCode();
    this._error = "";
  ...
  // Postcode-gedrag per vervoerder:
  ...
  _formatDate(value) {
    ...
    if (isNaN(date.getTime()) && /^\d+$/.test(String(value))) {
      // Numerieke timestamp: onderscheid seconden (10 cijfers) vs
      // milliseconden (13 cijfers), want vervoerders geven dit niet
      // altijd consistent door.
      const num = Number(value);
      date = new Date(String(value).length <= 10 ? num * 1000 : num);
    }
    if (isNaN(date.getTime())) {
      return String(value); // onbekend formaat: toon de ruwe waarde i.p.v. "Invalid Date"
    }
  ...
  async _submit() {
    ...
    if (this._postalCodeMode(this._carrier) === "required" && !this._postalCode.trim()) {
    ...
    try {
      ...
      this._saveLastPostalCode(this._postalCode.trim());
      this._trackingNumber = "";
      // Postcode NIET leegmaken: meestal hetzelfde adres voor het
      // volgende pakje, dus laten staan voor het gemak.
      this._error = "";
    } catch (err) {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <ha-card>
        ...
        ${
          this._postalCodeMode(this._carrier)
            ? `<div class="field">
                 <label>Postcode ontvanger${this._postalCodeMode(this._carrier) === "optional" ? " (optioneel, verbetert nauwkeurigheid)" : ""}</label>
